...
#include "byopen.h"
    ...
    static handle byopen_handles[4096] = {0};

    static handle open(char const *path, bool global, bool bypass) {
        if (bypass) {
            handle hd = by_dlopen(path, BY_RTLD_LAZY);
            for (int i = 0; i < 4096; ++i) {
                if (byopen_handles[i] == 0) {
                    byopen_handles[i] = hd;
                    return hd;
                }
            }
    ...
    void close(c_lib *cl, lua_State *L) {
        ...
        if (cl->h != FFI_DL_DEFAULT) {
            int bypass = 0;
            for (int i = 0; i < 4096; ++i) {
                if (byopen_handles[i] == cl->h) {
                    byopen_handles[i] = 0;
                    bypass = 1;
                    break;
                }
            }
            if (bypass && cl->h != FFI_DL_DEFAULT) {
                by_dlclose(cl->h);
            } else {
                dlclose(cl->h);
            }
...
/* ldscript handling logic generally adapted from luajit... */
    ...
    void load(c_lib *cl, char const *path, lua_State *L, bool global, bool bypass) {
        if (!path) {
            /* primary namespace */
            cl->h = FFI_DL_DEFAULT;
        ...
        handle h = open(resolve_name(L, path), global, bypass);
        lua_pop(L, 1);
        if (h) {
            lua::mark_lib(L);
            cl->h = h;
        ...
        if (
            ...
            h = open(lua_tostring(L, -1), global, bypass);
            lua_pop(L, 1);
            if (h) {
                lua::mark_lib(L);
                cl->h = h;
