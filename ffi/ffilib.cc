...
#undef FIELD_CHECK
    ...
    static int load_f(lua_State *L) {
        ...
        bool bypass = (lua_gettop(L) >= 3) && lua_toboolean(L, 3);
        auto *c_ud = static_cast<lib::c_lib *>(
                lua_newuserdata(L, sizeof(lib::c_lib))
        );
        new(c_ud) lib::c_lib{};
        lib::load(c_ud, path, L, glob, bypass);
        return 1;
    }
    ...
    static void setup(lua_State *L) {
        ...
        Dl_info info;
        dladdr((void *) luaL_tolstring, &info);
        lua_newtable(L);
        lua_pushstring(L, info.dli_fname);
        lua_setfield(L, -2, "dli_fname");
        lua_pushlightuserdata(L, info.dli_fbase);
        lua_setfield(L, -2, "dli_fbase");
        lua_pushlightuserdata(L, L);
        lua_setfield(L, -2, "ptr");
        lua_setfield(L, -2, "L");
        dladdr((void *) FIND__LOCATE__LIBNAME__, &info);
        lua_newtable(L);
        lua_pushstring(L, info.dli_fname);
        lua_setfield(L, -2, "dli_fname");
        lua_pushlightuserdata(L, info.dli_fbase);
    ...
    static void open(lua_State *L) {
        ...
        new(c_ud) lib::c_lib{};
        lib::load(c_ud, nullptr, L, false, false);
        lib_meta::setup(L);
    }
};
